...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on('joined_queue', (data: { message: string }) => {
      logger.info('Joined queue:', data);
      setQueueState(prev => ({ ...prev, isInQueue: true }));
      setResponse(data.message);
      setLoading(false);
    });

    // Listen for leave confirmations
    socket.on('left_queue', (data: { message: string }) => {
      logger.info('Left queue:', data);
      setQueueState(prev => ({ ...prev, isInQueue: false, items: [] }));
      setPosition(null);
      setResponse(data.message);
      setLoading(false);
    });
  ...
  return (
    <div className="dashboard-container">
      ...
      {position && <div className="queue-position">Position in Queue: {position}</div>}
